...
if (!$card_id) {
    ...
    ////////////////////////////เช็คcard_id
 // ตรวจสอบสถานะของ card_id ก่อน
$stmt = $conn->prepare("SELECT status_id FROM card WHERE card_id = ?");
$stmt->bind_param("i", $card_id);
$stmt->execute();
...
if ($result_status->num_rows > 0) {
    ...
    $card_status = $row_status['status_id'];

    // ตรวจสอบว่า status_id เป็น 1 หรือไม่
    if ($card_status != 1) {
        // ถ้าไม่ใช่ 1 แสดงข้อความว่าจองแล้วและหยุดการทำงาน
        $parking_slot = "Card ID นี้จองแล้ว";
        $bay_name = "N/A";
    } else {
        // ตรวจสอบว่ารถสูงเกิน 190 หรือไม่
        if ($distance > 190) {
...
<div class="road">
    ...
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="accept_return.php" class="menu-item ">Update Status</a></li>
            <li><a href="moniter.php" class="menu-item" target="_blank">Monitor</a></li>
            <li><a href="index.php" class="menu-item">Sign out</a></li>
        </ul>
        <div class="version">
            <p>Version 2.0</p>
        </div>

...
$stmt = $pdo->query("
    ...
    INNER JOIN lot ON card.lot_id = lot.lot_id 
    INNER JOIN bay ON lot.bay_id = bay.bay_id  -- JOIN ตาราง bay
    WHERE lot.status_id = '6'   -- ใช้ IN เพื่อดึงค่า status_id ที่เป็น 6 หรือ 7
    ORDER BY card.time DESC 
    LIMIT 10
");
$cards = $stmt->fetchAll();
...
<meta charset="UTF-8">
    ...
    <style>
        ...
        background: linear-gradient(90deg, #ff3e00, #00f7ff); 
...
<script>
    // ฟังก์ชันสำหรับดึงข้อมูลจาก fetch_parking_info.php
    ...
    // เรียกใช้ฟังก์ชันนี้ทุกๆ 5 วินาที
    setInterval(fetchParkingInfo, 5000);

    // ดึงข้อมูลทันทีเมื่อหน้าโหลดเสร็จ
